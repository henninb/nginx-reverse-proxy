...
5. **Language-specific patterns to check** (apply whichever is relevant to the language in scope):
   ...
   - **Nginx / Docker**: `proxy_pass` to user-controlled upstream URLs (SSRF), `add_header Access-Control-Allow-Origin *` allowing any origin, HTTP upstream in `proxy_pass` instead of HTTPS, missing `ssl_verify_client` for mutual TLS on internal services, `client_max_body_size` not set enabling unbounded upload DoS, `server_tokens off` not set leaking the nginx version string, TLS private key files (`.pem`, `.key`) committed to version control, `proxy_set_header Host $http_host` forwarding attacker-controlled Host headers, missing `limit_req_zone` / `limit_conn_zone` rate limiting, Docker images using `latest` tag (unpinned), secrets in Dockerfile `ENV` directives baked into image layers, `allow all` without a preceding `deny all` in location blocks
