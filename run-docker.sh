...
# Set DOCKER_HOST to remote server before running docker commands
export DOCKER_HOST=ssh://henninb@192.168.10.10

# Create docker context (suppress error if already exists)
docker context create remote --docker "host=ssh://henninb@192.168.10.10" 2>/dev/null || true

docker build -t nginx-reverse-proxy .
docker rm -f nginx-reverse-proxy 2>/dev/null || true
docker run --name=nginx-reverse-proxy -h nginx-reverse-proxy --restart unless-stopped -p 443:443 -p 2223:2223 -d nginx-reverse-proxy
docker ps -a
